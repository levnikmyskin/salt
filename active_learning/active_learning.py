...
class ActiveLearning:
    ...
    def run(self, budget: int, pool_size=100_000) -> \
        ...
        while self.__current_generated_len([train_idx_list, val_idx_set]) < budget:
            batch = self.estimator.get_next_batch(self.x, self.y, train_idx_list, val_idx_list, self.batch_strat.next_batch_size())

            for strat in filter(lambda s: str(s) not in self.stops, self.stopping_strats):
                if strat.should_stop(self.x, self.y, train_idx_list, val_idx_list, np.copy(self.estimator.probas), i):
                    self.stops[str(strat)] = Stop(i, (self.y[train_idx_list].sum() + self.y[val_idx_list].sum()) / self.y.sum(), strat.target_recall)

            train_idx_list.extend(take(self.batch_strat.current_batch_size(),
                                       filter(lambda i: i not in train_idx_set and i not in val_idx_set, batch)))
            train_idx_set.update(train_idx_list)

            if (((self.y[train_idx_list].sum() + self.y[val_idx_list].sum()) == self.y.sum()) and self.stop_when_no_pos) or len(self.stops) == self.stopping_strats:
                break

            i += 1

        for strat in filter(lambda s: str(s) not in self.stops, self.stopping_strats):
            self.stops[str(strat)] = Stop(i, 1., strat.target_recall)
        return train_idx_list, val_idx_list, list(set(np.arange(len(self.y))) - train_idx_set - val_idx_set)
