...
from utils.data_utils import (
    ALPolicy,
    flatten,
    random_sample_from_dataset,
    get_xy_with_given_pos_and_negs,
)
...
def lsa_plot(policy: ALPolicy, cls, y_c, x_svd):
    files = sorted(
        glob.glob(os.path.join(args.load_path, cls, f"*{policy}*pool1*")),
        key=lambda f: int(f.split("checkpoint-")[-1].split("_")[0]),
    )
    initial_idxs = []
    for f in files:
        with open(f, "rb") as o:
