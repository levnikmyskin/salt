...
class RecallPlotListener(StoppingListener):
    ...
    def on_estimated_recall(self, x: Inputs, y: Labels, tr_idxs: Indices, val_idxs: Indices, est_recall: List[float],
                            iteration: int):
...
class DistribPlotListener(RecallPlotListener):
    ...
    def on_estimated_recall(self, x: Inputs, y: Labels, tr_idxs: Indices, val_idxs: Indices, est_recall: List[float],
        ...
        x_rand_tr, y_rand_tr, _, _ = random_dataset_with_given_prevalences(x, y, y[tr_idxs].mean(), y[test_idxs].mean(),
                                                                           len(tr_idxs), len(test_idxs))
        ...
        sns.histplot({r'AL $\mathrm{Pr}(y=1|x)$': est_probs, r'$Rand \ \mathrm{Pr}(y=1|x)$': true_probs},
