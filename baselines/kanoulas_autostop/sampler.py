...
class HorvitzThompson:
    ...
    def _mask_sampled_dids(self, sampled_state):
        first_order_mask = np.array(
            [1 if did in sampled_state else 0 for did in self.complete_dids]
        )[None, :]
    ...
    def _sample(self, ranked_dids, dist, n, replace):
        """Sample n items"""
        assert len(ranked_dids) == len(dist), "dids len != dist len ({} != {})".format(
            len(ranked_dids), len(dist)
        )
        return np.random.choice(
            a=ranked_dids, size=n, replace=replace, p=dist
        )  # returned type is the same with ranked_dids
    ...
    def estimate(self, t, stopping_condition, sampled_state):
        ...
        first_order_inclusion_probabilities = 1.0 - np.exp(
            self.cumulated_prod_1st_order
        )
        ...
        total = np.sum(
            first_order_mask
            * (self.complete_labels / first_order_inclusion_probabilities)
        )
        ...
        if t > 1:
            variance1, variance2 = (
                -1,
                -1,
            )  # no need to calculate variance if stopping condition is loose

            if stopping_condition == "strict1" or stopping_condition == "strict2":
                M = np.tile(first_order_inclusion_probabilities, (self.N, 1))
                MT = M.T
                second_order_inclusion_probabilities = (
                    M + MT - (1.0 - np.exp(self.cumulated_prod_2nd_order))
                )
                ...
                if stopping_condition == "strict1":
                    ...
                    # 1/pi_i**2 - 1/pi_i
                    part1 = (
                        1.0 / first_order_inclusion_probabilities**2
                        - 1.0 / first_order_inclusion_probabilities
                    )
                    # y_i**2
                    ...
                    np.fill_diagonal(
                        part2, 0.0
                    )  # set diagonal values to zero, because summing part2 do not include diagonal values
                    ...
                    variance1 = np.sum(first_order_mask * part1 * yi_2) + np.sum(
                        second_order_mask * part2 * yi_yj
                    )
                    ...
                    if v == 1:
                        ...
                        variance2 = (
                            ...
                            * self.complete_labels
                            / first_order_inclusion_probabilities
                            - total
                        ) ** 2
                        variance2 = (
                            (self.N - v)
                            ...
                            / v
                            / (v - 1)
                            * np.sum(first_order_mask * variance2)
                        )
...
class HTAPPriorSampler(HorvitzThompson):
    ...
    def update_distribution(self):
        dist = [
            math.log((self.N + 1) / (i + 1)) for i in np.arange(self.N)
        ]  # i+1 from 1 to N
        summ = sum(dist)
