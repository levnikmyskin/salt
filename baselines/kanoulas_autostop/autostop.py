...
class AutoStopAnnotatingPolicy(BaseAnnotatingPolicy):
    ...
    def get_next_batch(
        self, x: Inputs, y: Labels, tr_idxs: Indices, _: Indices, size: int
    ) -> Indices:
        sorted_idxs = self.rank_documents(x, y, tr_idxs, _)
        self.current_iter += 1
        return np.unique(
            self.sampler.sample(None, sorted_idxs, size, self.stopping_condition)
        )
    ...
    def should_stop(
        self, x: Inputs, y: Labels, train_idxs: Indices, _: Indices
    ) -> bool:
        total_esti_rels, var_1, var_2 = self.sampler.estimate(
            self.current_iter, self.stopping_condition, set(train_idxs)
        )
        ...
        match self.stopping_condition:
            ...
            case "strict1":
                return current_rels >= self.target_recall * (
                    total_esti_rels + np.sqrt(var_1)
                )
            case "strict2":
                return current_rels >= self.target_recall * (
                    total_esti_rels + np.sqrt(var_2)
                )
        return False
...
class SLDAutoStopAnnotatingPolicy(AutoStopAnnotatingPolicy):
    def rank_documents(
        ...
        self.sld_posteriors, _ = self._get_sld_posteriors(
            x, y, train_idxs, test_idxs, val_idxs, preds
        )
