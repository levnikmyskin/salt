...
def plot_cluster_with_pos(title, x_svd, cluster_preds, y, idx_to_plot, idx_annotated):
    if idx_to_plot:
        ...
        plt.scatter(x_svd[idx_to_plot, 0], x_svd[idx_to_plot, 1], c=cluster_preds[idx_to_plot], alpha=0.2)
    ...
    if idx_annotated is not None:
        ...
        plt.scatter(x_svd[idx_annotated, 0][y[idx_annotated] == 1], x_svd[idx_annotated, 1][y[idx_annotated] == 1], c='blue')
    plt.title(title)
    plt.show()


def run_on_fake_data(sample_size=10_000):
    x, y = make_classification(n_samples=sample_size, n_features=2, n_redundant=0, n_informative=2, n_classes=4,
                               n_clusters_per_class=1, class_sep=1, weights=[0.1, 0.4, 0.1, 0.4])
    clusters = y
    y_c = np.copy(y)
    ...
    test_idxs = list(set(np.arange(len(y_c))) - set(train_idxs))
    rand_tr, rand_te = random_dataset_with_given_prevalences(x, y_c, y_c[train_idxs].mean(), y_c[test_idxs].mean(), len(train_idxs), len(test_idxs), return_idxs=True)
    ...
    for c in set(clusters):
        ...
        ax1.scatter(x[train_idxs, 0][clusters[train_idxs] == c], x[train_idxs, 1][clusters[train_idxs] == c],
                    marker='x',
                    c=tr_colors[c],
                    alpha=tr_alphas[c])
        ax2.scatter(x[test_idxs, 0][cl_ids], x[test_idxs, 1][cl_ids], c=colors[c], alpha=0.2)
        ax2.scatter(x[rand_tr, 0][rand_cl], x[rand_tr, 1][rand_cl], marker='x', c=tr_colors[c], alpha=tr_alphas[c])
    ...
    ax1.set_title('$P_{U}(y) = %.3f; \hat{P}_{U}^{\mathrm{AL}}(y) = %.3f; \hat{P}_{U}^{Rand}(y) = %.3f$'
                  % (y_c[test_idxs].mean(), al_probs[test_idxs, 1].mean(), rand_probs[test_idxs, 1].mean()))
