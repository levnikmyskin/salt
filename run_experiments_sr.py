...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--target-recall', nargs='+', type=float, help='target recall TAR should stop at',
                        required=True)
        ...
        for i in range(args.runs):
            for topic in topics:
                ...
                with open(path, 'rb') as f:
                    x, y = pickle.load(f)
                stoppings = [cormack_knee.KneeStopping(target_recall=args.target_recall, min_rounds=0),
                ...
                for t in args.target_recall:
                    ...
                    adj_sld = SLDQuantStopping(target_recall=t, nstd=0., dataset_length=len(y), min_rounds=0)
                    adj_sld_m = SLDQuantStopping(target_recall=t, nstd=0., dataset_length=len(y), use_margin=True)
                ...
                futures.append(p.submit(run_al, topic_name, len(y), x, y, len(y),
                                        copy.deepcopy(policy), stoppings))
