...
from utils.metrics import normalized_absolute_error, mean_square_error, relative_error, YangIdealizedCost
...
def latex_table_it_recall(stoppings, target_rec=0.8):
    for k, v in res.items():
        ...
        for ss in stoppings:
            if str(ss) not in s:
                s[str(ss)] = {'it': 100, 'recall': 1.}
            s[str(ss)]['it'] = s[str(ss)]['it'] if type(s[str(ss)]['it']) is int else 100
            s[str(ss)]['cost'] = cost.evaluate(LinearStrategy(b=100), v['idxs'][0], v['y_c'], s[str(ss)]['it'], target_rec)
        s['class'] = k
        v['stops'] = s
    df = pd.DataFrame([v['stops'] for v in res.values()])
    buf = []
    order = ['Knee', f'QuantCI 0 @ {target_rec}', f'QuantCI 2.0 @ {target_rec}', f'SLDQuant 0.0 @ {target_rec}', f'SLDQuant 1 @ {target_rec}', f'SLDQuant 2 @ {target_rec}']
    ...
    return temp.substitute(class_results=' \\\\ \n'.join(buf), avg_results=' & '.join(f'{round(avgs[o][k], 3) if k == "recall" else round(avgs[o][k], 0)}' for o in order for k in ['cost', 'recall']))
...
def latex_results(df, clss, exclude, target_rec=.8, exclude_costs=True, precision=3):
    ...
    return pd.DataFrame(new_d).sort_index().style.format(precision=precision).format_index(lambda i: mapper.get(i, i)).\
        apply(bold_best_it_second).to_latex(hrules=True)
...
def boxplots(res, target_rec, legend_loc='lower right', d_name='RCV1-v2'):
    ...
    mapper = {f'SLDQuant 0.0 & a=0.3 @ {target_rec:.2f} w\\ m=0': r'$\mathrm{SAL}_\tau$ (ours)',
              f'SLDQuant 0.0 & a=0.3 @ {target_rec:.2f} w\\ m=1': r'$\mathrm{SAL}_\tau^{R}$ (ours)',
              f'QuantCI 2.0 @ {target_rec:.2f}': 'QuantCI',
              f'CHM @ {target_rec:.2f}': 'CHM', 'Knee': 'Knee', 'BudgetKnee': 'Budget'}
...
if __name__ == '__main__':
    ...
    cost_structs = [YangIdealizedCost(), YangIdealizedCost.with_expensive_training(), YangIdealizedCost.with_minecore_like()]
