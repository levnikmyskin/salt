...
if __name__ == '__main__':
    ...
    for t in args.target_recall:
        for alpha in args.alpha:
            stoppings.append(
                SLDQuantStopping(nstd=0., target_recall=t, dataset_length=pool_size, alpha=alpha)
            )
    ...
    with ProcessPoolExecutor(max_workers=jobs) as p:
        ...
        for topic, x, y in dataset:
            ...
            for t in args.target_recall:
                for alpha in args.alpha:
                    stoppings.append(
                        SLDQuantStopping(nstd=0., target_recall=t, dataset_length=len(y), alpha=alpha)
                    )
            futures.append(p.submit(run_al, topic, len(y), x, y,
                                    len(y), copy.deepcopy(policy), copy.deepcopy(stoppings)))
